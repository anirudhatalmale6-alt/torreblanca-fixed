...
(() => {
  ...
  const bust = (u) => u + (u.includes("?") ? "&" : "?") + "v=" + CACHE_BUST;
  ...
  function imgUrl(areaKey, i) {
    return bust(`${ASSETS}${APT}-${areaKey}-${i}.jpg`);
  }
  ...
  async function loadDocxHtml(areaKey) {
    const docx = bust(`${ASSETS}${APT}-${areaKey}-txt.docx`);
  ...
  function setOverlaySrc(src) {
    if (!overlay) return;
    overlay.src = bust(src);
    overlay.style.opacity = src && src !== BASE_IMG ? "1" : "0";
  }

  function resetOverlay() {
    setOverlaySrc(bust(BASE_IMG));
